...
const ITEMS: Array<{ key: keyof Layers; label: string; cls: string }> = [
  { key: 'transporte', label: 'Transporte', cls: 'bg-cyan' },
...
export function MapLayersControl({ layers, onChange }: { layers: Layers; onChange: (l: Layers) => void }) {
  return (
    <div className="absolute bottom-3 right-3 z-[1000] rounded-md border border-line bg-panel/85 backdrop-blur px-3 py-2">
      <div className="text-[11px] uppercase tracking-wider text-muted mb-1">Capas del mapa</div>
      {ITEMS.map((it) => (
        <label key={it.key} className="flex items-center gap-2 py-0.5 text-[12px] cursor-pointer">
          <button role="switch" aria-checked={layers[it.key]} onClick={() => onChange({ ...layers, [it.key]: !layers[it.key] })} className={`w-8 h-4 rounded-full relative transition-colors ${layers[it.key] ? it.cls : 'bg-white/10'}`}>
