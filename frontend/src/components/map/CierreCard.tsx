  ...
  return (
    <Glass label="Resultado" className={className}>
      <div className="flex items-center gap-2 px-4 pt-3 pb-1">
        {stuck ? <AlertTriangle size={14} className="text-red" /> : <CheckCircle2 size={14} className="text-green" />}
        <span className={`label ${stuck ? 'text-red' : 'text-green'}`}>{stuck ? 'Plan incompleto' : 'Plan cerrado'} · v{s.planVersion}</span>
      </div>
      {stuck && s.closureSummary && <p className="px-4 pb-2 text-[12px] text-amber">{s.closureSummary}</p>}
      <div className="px-4 pb-3 grid grid-cols-3 gap-3">
        <Stat value={`${k.confirmed}/${k.total}`} label="invitados con sede" />
