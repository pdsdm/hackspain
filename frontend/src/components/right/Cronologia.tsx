          ...
          return (
            <li key={e.id} className={`flex gap-2.5 py-1.5 ${latest ? 'fade-in' : ''}`}>
              <span className="flex flex-col items-center pt-1.5">
                <span className={`w-2 h-2 rounded-full flex-none ${EVENT_DOT[e.kind]} ${latest ? 'animate-pulse' : ''}`} />
                <span className="w-px flex-1 bg-line mt-1" />
              </span>
              <div className="min-w-0">
                <span className="text-[11px] text-muted num mr-2">{fmtClock(e.time)}</span>
                <span className={`text-[12px] ${e.kind === 'fallo' || e.kind === 'incidencia' ? 'text-red/90' : e.kind === 'intervencion' ? 'text-white font-medium' : e.kind === 'decision' || e.kind === 'espera' ? 'text-amber' : latest ? 'text-text' : 'text-text/85'}`}>{e.text}</span>
              </div>
            </li>
          )
        })}
