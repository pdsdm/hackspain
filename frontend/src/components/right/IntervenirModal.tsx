    ...
    <div className="fixed inset-0 z-[2000] bg-black/60 backdrop-blur-sm grid place-items-center p-4" onClick={onClose}>
      <div className="w-full max-w-md bg-panel border border-line rounded-lg p-4 fade-in" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-3">
          <h3 className="font-semibold text-[15px]">Intervenir · responsable de operaciones</h3>
        ...
        <div className="space-y-2">
          ...
          {s.agentsPaused
            ? <Btn icon={Play} label="Reanudar agentes" sub="Los agentes retoman sus tareas" tone="text-cyan" onClick={() => fire({ type: 'resume' })} />
            : <Btn icon={Pause} label="Pausar nuevas acciones" sub="Los agentes no inician nuevas llamadas ni reservas" tone="text-cyan" onClick={() => fire({ type: 'pause' })} />}
          <Btn icon={PhoneForwarded} label="Hacerme cargo de la conversación" sub="Tomar la llamada en curso" tone="text-cyan" onClick={() => fire({ type: 'take_call' })} />
          <div className="p-3 rounded-md border border-line">
            ...
            <div className="flex gap-2 mt-2">
              <input value={text} onChange={(e) => setText(e.target.value)} placeholder="p. ej. «Nadie cruza a Norte sin lanzadera»" className="flex-1 h-9 px-2 rounded-md bg-bg border border-line text-[12px] outline-none focus:border-amber/60" />
              <button disabled={!text.trim()} onClick={() => fire({ type: 'set_constraint', payload: { text: text.trim() } })} className="h-9 px-3 rounded-md border border-amber/50 text-amber disabled:opacity-40">Fijar</button>
            </div>
