...
import { spaceLook, COMMITMENT } from '../ui/status'
...
export function ResourceDetail({ s, id, changes, onClose }: { s: CrisisState; id: string; changes: StateChange[]; onClose: () => void }) {
  ...
  const title = space?.name ?? shuttle?.name ?? delivery?.name ?? vehicle?.name ?? commitment?.title ?? group?.name ?? decision?.title ?? 'Detalle del recurso'
  ...
  return <Dialog title={title} onClose={onClose}>
    <div className="space-y-3 text-[14px]">
      ...
      {vehicle && <><p>{vehicle.who} · {vehicle.count} {vehicle.kind === 'repartidor' ? 'vehículo' : 'personas'} · {vehicle.origin} → {place(vehicle.destinationId)}</p><p>{vehicle.status.replaceAll('_', ' ')} · llegada {fmtClock(vehicle.arriveAt)} · contacto {vehicle.counterpart}</p><p>{vehicle.note}</p></>}
