...
Reset autenticado para el ensayo real en producción. Crea un run `calm`, desactiva el Modo vivo, acelera el reloj y marca `forceSimActions: true`; aunque Railway tenga hooks de especialistas, llamadas, SMS y email usan el adaptador `sim` durante ese run. Un reset normal elimina la marca. Puede recibir `inputTokenHash`, SHA-256 del bearer de un workflow publicado desactualizado; solo ese run aislado lo acepta en `/workflow/happyrobot/events` y nunca se guarda el token en claro.
