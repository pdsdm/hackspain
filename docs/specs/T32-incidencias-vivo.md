...
- [x] **Modo abierto** (por defecto con LLM): un agente mundo (`backend/src/domain/incident-generator.ts`) inventa cada incidencia a partir del estado real y de una pista de la semilla; devuelve texto + hasta 3 operaciones validadas. Sin LLM o si falla, catálogo.
