...
- [x] El extractor traduce el resultado de `POST /workflow/results` a `data.guestGroups[]` con `informedCount` y `acceptedCount`, y el backend los aplica a `guestGroups` en `/state`. «Enviado», «entregado» y «aceptado» son cuentas distintas.
- [x] Un mensaje que HappyRobot no confirma como entregado no suma a `informedCount`.
- [ ] Con `HAPPYROBOT_HOOK_ASISTENTES` y clave, un SMS llega a un móvil del equipo y su resultado aparece en `/state`. Sin hook o sin `HAPPYROBOT_API_KEY`, la tarea termina `failed` con «Sin canal real configurado para asistentes».
