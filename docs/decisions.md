...
- **Circuito (T54):** se piden `alternatives=3` y se elige la ruta que menos metros comparte con el trazado (`TRACK` y `PIT_LANE`, corredor de 25 m). Cruzar el trazado por un túnel o puente suma pocos metros y no penaliza; ir en paralelo sí. Si la mejor sigue compartiendo más de 150 m, se prueban dos puntos de paso a 300 m a cada lado del tramo solapado y se conserva la mejor. Las peticiones van en cola (2 en vuelo) para no saturar el servidor público. Varios destinos (Muelle Este, Parking Norte, Muelle Norte) están pegados al trazado, así que su aproximación final puede seguir compartiendo calle.
