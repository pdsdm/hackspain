...
def start_transfer(customer_call_sid: str, motivo: str, to: Optional[str] = None) -> dict:
    """
    Monta la sala de tres: agente IA + cliente + humano.

    Orden deliberado: primero entra la IA (su leg lo crea Twilio al instante y arranca
    la conferencia), luego se redirige al cliente, y por último se llama al humano, que
    es lo único que tarda en descolgar. Así el cliente nunca se queda en silencio.
    """
    destination = to or HUMAN_PHONE_NUMBER
    if not destination:
        raise HTTPException(status_code=400, detail="Falta HUMAN_PHONE_NUMBER o el parámetro 'to'")
    ...
    # 3) Se llama al humano, que entra cuando descuelga.
    ...
    conf["participants"].append({"label": "human", "call_sid": human.call_sid, "to": destination})
    conf["status"] = "esperando_humano"

    print(f"🔀 Sala {conference_name}: IA {conf['ai_call_sid']} · humano {human.call_sid} → {destination}")
    return conf
...
async def add_participant(conference_name: str, to: Optional[str] = None,
                          label: str = "extra",
                          x_demo_token: Optional[str] = Header(default=None)):
    """Añade a otra persona a una conferencia ya creada. Repetible: sala de N personas."""
    require_token(x_demo_token)
    destination = to or HUMAN_PHONE_NUMBER
    if not destination:
        raise HTTPException(status_code=400, detail="Falta HUMAN_PHONE_NUMBER o el parámetro 'to'")
    ...
    if conf:
        conf["participants"].append({"label": label, "call_sid": participant.call_sid, "to": destination})
    return {"status": "ok", "call_sid": participant.call_sid, "label": label}
